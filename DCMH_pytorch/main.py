...
from models import ImgModule, TxtModule, TxtGcnnet
...
def train(**kwargs):
    ...
    txt_model = TxtGcnnet(y_dim, opt.bit)
    if opt.use_gpu:
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i in tqdm(range(num_train // batch_size)):
            ...
            sample_L = Variable(train_L[ind, :])
            text = train_y[ind, :].unsqueeze(-1).type(torch.float)
        ...
        if opt.valid:
            ...
            if mapt2i >= max_mapt2i and mapi2t >= max_mapi2t:
                max_mapi2t = mapi2t
                max_mapt2i = mapt2i
                img_model.save(img_model.module_name + '_gcn.pth')
                txt_model.save(txt_model.module_name + '_gcn.pth')
...
def generate_text_code(txt_model, Y, bit):
    ...
    for i in tqdm(range(num_data // batch_size + 1)):
        ind = index[i * batch_size: min((i + 1) * batch_size, num_data)]
        text = Y[ind].unsqueeze(-1).type(torch.float)
        if opt.use_gpu:
            text = text.cuda()
...
def write_result(result):
    import os
    with open(os.path.join(opt.result_dir, 'result_gcn.txt'), 'w') as f:
