...
from models import ImgModule, TxtModule, HashModule
...
def train(**kwargs):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i in tqdm(range(num_train // batch_size)):
            ...
            Y_fea = Variable(Y_fea_buffer)
            G = Variable(G_buffer)
            B_x = torch.sign(F_buffer)
            
            theta_x = 1.0 / 2 * torch.matmul(cur_f, G.t())
            logloss_x = -torch.sum(S * theta_x - torch.log(1.0 + torch.exp(theta_x)))
            #theta_x_fea = 1.0 / 2 * torch.matmul(image_fea, Y_fea.t())
            #logloss_x_fea = -torch.sum(S * theta_x_fea - torch.log(1.0 + torch.exp(theta_x_fea)))
            quantization_x = torch.sum(torch.pow(B_buffer[ind, :] - cur_f, 2))
            labelloss_x = torch.sum(torch.pow(train_L[ind, :].float() - image_label,2))
            loss_F = torch.torch.sum(torch.pow(B_x[ind, :]- cur_f, 2))
            loss_x = logloss_x + opt.gamma * quantization_x + opt.eta * labelloss_x + loss_F # + logloss_x_fea 
        ...
        for i in tqdm(range(num_train // batch_size)):
            ...
            B_y = torch.sign(G_buffer)
            F = Variable(F_buffer)
            X_fea = Variable(X_fea_buffer)
            # calculate loss
            ...
            #theta_y_fea = 1.0 / 2 * torch.matmul(txt_fea, X_fea.t())
            #logloss_y_fea = -torch.sum(S * theta_y_fea - torch.log(1.0 + torch.exp(theta_y_fea)))
            quantization_y = torch.sum(torch.pow(B_buffer[ind, :] - cur_g, 2))
            labelloss_y = torch.sum(torch.pow(train_L[ind, :].float() - txt_label,2))
            loss_G = torch.sum(torch.pow(B_y[ind, :]- cur_g, 2))
            loss_y = logloss_y + opt.gamma * quantization_y + opt.eta * labelloss_y + loss_G # + logloss_y_fea
        ...
        for i in tqdm(range(num_train // batch_size)):
            ...
            loss_hash = logloss_hash + label_loss + hashloss
        ...
        loss, hash_loss, total_loss = calc_loss(B, F, G, Variable(Sim), opt.gamma, opt.eta,Label_buffer, train_L, X_label_buffer,Y_label_buffer)
